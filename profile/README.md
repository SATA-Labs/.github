<div align="center">

<!-- header -->
<img src="https://satalabs.xyz/sata-logo-email.png" alt="SATA Labs" width="80" />

# SATA LABS

**`Super Awesome Team Awesome`**

audio plugins. software tools. weird visualizers. good vibes only.

[![site](https://img.shields.io/badge/satalabs.xyz-000?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white)](https://satalabs.xyz)
&nbsp;
[![glitch-mesh](https://img.shields.io/badge/glitch--mesh_live_demo-111?style=for-the-badge&logo=github&logoColor=white)](https://sata-labs.github.io)

---

</div>

### `> what we do`

we build audio tools because we like building things. two people, no corporate nonsense, just code and sound.

our stuff ships on **[satalabs.xyz](https://satalabs.xyz)** — that's home base.

### `> current gear`

| project | what it is | status |
|---|---|---|
| **[LPF9000](https://satalabs.xyz/products/lpf9000)** | dj-style EQ & filter plugin. 3 knobs. 2 modes. analog warmth. no frills. | `free` `shipped` |
| **[glitch-mesh](https://sata-labs.github.io)** | real-time generative mesh visualizer w/ MIDI learn, psychedelia fields, feedback loops | `live` |

### `> stack`

<div>

![Next.js](https://img.shields.io/badge/Next.js-000?style=flat-square&logo=nextdotjs)
![React](https://img.shields.io/badge/React-000?style=flat-square&logo=react)
![TypeScript](https://img.shields.io/badge/TypeScript-000?style=flat-square&logo=typescript)
![Tailwind](https://img.shields.io/badge/Tailwind-000?style=flat-square&logo=tailwindcss)
![Cloudflare](https://img.shields.io/badge/Cloudflare_Workers-000?style=flat-square&logo=cloudflare)
![Supabase](https://img.shields.io/badge/Supabase-000?style=flat-square&logo=supabase)
![Stripe](https://img.shields.io/badge/Stripe-000?style=flat-square&logo=stripe)
![Canvas API](https://img.shields.io/badge/Canvas_API-000?style=flat-square&logo=html5)
![Web MIDI](https://img.shields.io/badge/Web_MIDI-000?style=flat-square&logo=midi)

</div>

### `> the deal`

50%+ of all proceeds go to charity. currently supporting [**SBSK**](https://www.specialbooks.org/) — promoting inclusion and understanding for people with disabilities. every download funds something bigger than a plugin.

---

<div align="center">

**[satalabs.xyz](https://satalabs.xyz)** &nbsp;&bull;&nbsp; **[glitch-mesh demo](https://sata-labs.github.io)** &nbsp;&bull;&nbsp; **[LPF9000](https://satalabs.xyz/products/lpf9000)**

<sub>built by ryan & adriana &mdash; because making things is fun</sub>

</div>
